...
idNumber      long
idNumberOvr   byte,dim(4),over(idNumber)
realValue     real
...
boolVar       bool
   ...
   map
     ...
     module('bcp')
       ! called once to do some set up, returns an env. handle store the handle locally
       ClaBcpInit(),long,c
       ...
       ! bind a long variable to qa table column.
       ...
       ! strings they can vary in size so the extra parameter is needed.  for most
...
cs           cstring('Driver={{SQL Server Native Client 10.0};Server=server name here;Database=databse name here;trusted_connection=yes;')
tableName    cstring('schemaName.tableName here')
...
t         long
  ...
  ! close the connection and free the env handle
  ClaBcpKill(hEnv, hDbc)
